...
int main(){
	...
	int contador = 0;
	bool finalizar_conjunto = false; // flag
	...
	while(contador < ingresar_N_elementos && finalizar_conjunto == false){
		...
		if(conjunto_A_elemento == 0){
			...
			if(contador_de_ceros > 4){
				// Le aviso al flag que deberá finalizar la ejecución del bucle
		...
		if(conjunto_A_elemento > 0){
			...
			// diviendo la suma total de ellos 
			promedio_numeros_positivos = (sumatoria_numeros_positivos / contador_numeros_positivos);

			// Si el promedio total de los numeros naturales ingresado es
			// mayor a 6			
